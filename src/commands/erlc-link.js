...
module.exports = {
  ...
  async execute(interaction) {
    try {
      ...
      const error = createError(ErrorCodes.LINK_VERIFICATION_FAILED, `User: ${interaction.user.id}`, err);
      await logErrorToDiscord(error, { user_id: interaction.user.id, command: 'erlc-link', guild_id: interaction.guildId }, interaction.client);
