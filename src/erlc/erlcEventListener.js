...
const { getERLCClient, getRobloxUsername } = require('../handlers/erlcHandler');
...
async function processModerationCommand(client, guildId, logEntry) {
  try {
    ...
    for (const [memberId] of guild.members.cache) {
      const link = await getRobloxUsername(guildId, memberId);
      if (link && link.toLowerCase() === logEntry.actor?.toLowerCase()) {
